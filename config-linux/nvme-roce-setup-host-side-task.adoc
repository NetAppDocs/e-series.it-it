...
# ifup eth5
----
. Impostare il livello NVMe-of sull'host. Creare il seguente file in `/etc/modules-load.d/` per caricare `nvme-rdma` kernel e assicurarsi che il modulo kernel sia sempre attivo, anche dopo un riavvio:
+
[listing]
----
