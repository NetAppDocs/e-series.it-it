...
[NOTE]
====
Alcuni sistemi storage dispongono di una sola porta di gestione Ethernet out-of-band su ciascun controller.

Per le istruzioni di cablaggio specifiche per il tuo sistema storage, consulta link:../getting-started/getup-run-concept.html["Guida rapida per e-Series"^] e fai clic sul collegamento relativo al tuo sistema storage.
